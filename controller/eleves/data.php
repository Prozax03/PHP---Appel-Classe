<?php
    ...
    $req = $db->prepare("SELECT e.id, e.nom, e.prenom, idSexe, CONCAT(c.libelleCourt, ' - ', c.libelle) as 'classe', dateEntree, dateSortie FROM eleves e, sexes s, classes c WHERE e.idClasse = c.id AND e.idSexe = s.id AND DATE(NOW()) <= e.dateSortie ORDER BY c.ordre, e.nom, e.prenom;");
    $req->execute(array());
